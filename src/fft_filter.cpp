...
void FFTFilter::AddSignalBlock(const vector<float>& signal_block) {
  assert(
      signal_block.size()==filter_len_ && "Signal block size must match filter length");
  ...
  for (int i = 0; i < filtered_freq_domain_buffer_.size(); ++i) {
    filtered_freq_domain_buffer_[i].r = freq_domain_buffer[i].r
        * kernel_freq_domain_buffer_[i].r
        - freq_domain_buffer[i].i * kernel_freq_domain_buffer_[i].i;
    filtered_freq_domain_buffer_[i].i = freq_domain_buffer[i].r
        * kernel_freq_domain_buffer_[i].i
